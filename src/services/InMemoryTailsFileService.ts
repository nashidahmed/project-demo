...
export class InMemoryTailsFileService extends BasicTailsFileService {
  ...
  public async uploadTailsFile(
    ...
    return {
      tailsFileUrl: options.revocationRegistryDefinition.value.tailsHash,
    };
  }
  ...
  public async getTailsFile(
    ...
    try {
      ...
      const tailsFilePath = await this.getTailsFilePath(
        agentContext,
        tailsHash
      );
      agentContext.config.logger.debug(
        `Tails file for ${tailsLocation} ${
          tailsExists ? "is stored" : "is not stored"
        } at ${tailsFilePath}`
      );

      if (!tailsExists) {
        agentContext.config.logger.debug(
          `Retrieving tails file from URL ${tailsLocation}`
        );
        const fileSystem = agentContext.dependencyManager.resolve<FileSystem>(
          InjectionSymbols.FileSystem
        );
        await fileSystem.downloadToFile(tailsLocation, tailsFilePath);
        agentContext.config.logger.debug(
          `Saved tails file to FileSystem at path ${tailsFilePath}`
        );
      }
      ...
      agentContext.config.logger.error(
        `Error while retrieving tails file from URL ${tailsLocation}`,
        {
          error,
        }
      );
